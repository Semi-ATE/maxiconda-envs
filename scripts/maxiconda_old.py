...
def write_log(OS_CPU, PY, environment, message):
    logfile = os.path.join(REPO_ROOT, "recipes", OS_CPU, PY, environment, "meta.log")
...
#def run_solver(pkgs, PY, target=None, channels=["conda-forge"]):
def run_solver(implementation, environment, pkgs, channels=["conda-forge"]):
    """
    ...
    os.environ["CONDA_SUBDIR"] = OS_CPU
    ...
    if stdout.decode("utf-8").startswith("Encountered problems while solving."):
        ...
        write_log(OS_CPU, PY, environment, feedback)
    else:
        try:
...
def solve(implementation, environment):
    """
    Run conda solver dry run. 
    ...
    Returns
    -------
    directory where the 'meta.yaml' is written, "" if there was a problem.
    """
    OS_CPU, PY = implementation.split('_')
    ...
    if packages_to_remove:
        message = f"  excluding packages : {packages_to_remove}"
        print(message)
        write_log(OS_CPU, PY, environment, message)
    ...
    recipe_path = RECIPES_ROOT / OS_CPU / PY / environment
    recipe_fpath = recipe_path / "meta.yaml"
    ...
    while data == {}: 
        if not cleaned_primary_packages:
            message = f"  aborting : no packages to solve for."
            print(message)
            write_log(OS_CPU, PY, environment, message)
            retval = ""
            break

        data, feedback = run_solver(implementation, environment, cleaned_primary_packages)

        if data == {}:
            print(f"  Problem : {feedback}")
            package_to_remove = feedback.split("needed by")[1].strip()
            message = f"  Excluding package : '{package_to_remove}'"
            print(message)
            print(cleaned_primary_packages)
            cleaned_primary_packages.remove(package_to_remove)
            write_log(OS_CPU, PY, environment, message)
        else:
            all_packages = {}
...
if __name__ == '__main__':
    implementation = "osx-arm64_py38"
    environment = "cf-eng"

    print(solve(implementation, environment))
